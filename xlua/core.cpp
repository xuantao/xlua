...
namespace internal {
    ...
    static void MakeGlobal(State* s, const char* path) {
        ...
        if (s->state_.module_ && *s->state_.module_) {
            if (Is_G(path))
                snprintf(buf, kBuffCacheSize, "%s", s->state_.module_);
            else
                snprintf(buf, kBuffCacheSize, "%s.%s", s->state_.module_, path);
        } else {
            strcpy_s(buf, kBuffCacheSize, path);
        }
        ...
        const TypeDesc* Finalize() override {
            ...
            if (super) {
                ...
                if (data->caster.offset_2_top) {
                    ...
                    while (desc->super && !desc->super->caster.is_multi_inherit)
                        desc = desc->super;
